...
const CLIENT_CRM_OBJECT_METADATA_UNIVERSAL_IDENTIFIERS = [
...
const CLIENT_CRM_FIELD_METADATA_UNIVERSAL_IDENTIFIERS = [
  ...
  STANDARD_OBJECTS.person.fields.billingContactForClientAccountProfiles
    .universalIdentifier,
  STANDARD_OBJECTS.person.fields.clientStakeholderRoles.universalIdentifier,
];

const CLIENT_CRM_INDEX_UNIVERSAL_IDENTIFIERS = [
  ...getUniversalIdentifiers(STANDARD_OBJECTS.clientAccountProfile.indexes),
  ...getUniversalIdentifiers(STANDARD_OBJECTS.clientStakeholderRole.indexes),
];

const CLIENT_CRM_VIEW_UNIVERSAL_IDENTIFIERS = [
...
const CLIENT_CRM_VIEW_FIELD_GROUP_UNIVERSAL_IDENTIFIERS = [
  ...getUniversalIdentifiers(
    STANDARD_OBJECTS.clientAccountProfile.views
      .clientAccountProfileRecordPageFields.viewFieldGroups,
  ),
  ...getUniversalIdentifiers(
    STANDARD_OBJECTS.clientStakeholderRole.views
...
const CLIENT_CRM_VIEW_FIELD_UNIVERSAL_IDENTIFIERS = [
  ...getUniversalIdentifiers(
    STANDARD_OBJECTS.clientAccountProfile.views.allClientAccountProfiles
      .viewFields,
  ),
  ...getUniversalIdentifiers(
    STANDARD_OBJECTS.clientAccountProfile.views
      .clientAccountProfileRecordPageFields.viewFields,
  ),
  ...getUniversalIdentifiers(
    STANDARD_OBJECTS.clientStakeholderRole.views.allClientStakeholderRoles
      .viewFields,
...
@RegisteredWorkspaceCommand('2.23.0', 1784193573815)
@Command({
  name: 'upgrade:2-23:sync-client-crm-standard-objects',
  description:
    'Sync client CRM standard objects (clientAccountProfile, clientStakeholderRole) to existing workspaces',
})
export class SyncClientCrmStandardObjectsCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    const clientAccountProfileObjectMetadataForNavigation =
    ...
    const clientStakeholderRoleObjectMetadataForNavigation =
      getExistingOrStandardFlatEntityOrThrow<FlatObjectMetadata>({
        standardFlatEntityMaps:
          standardAllFlatEntityMaps.flatObjectMetadataMaps,
        existingFlatEntityMaps: flatObjectMetadataMaps,
    ...
    const navigationCommandMenuItemOperations =
      buildNavigationCommandMenuItemOperationsOrThrow({
        ...
        objectMetadatasForNavigation: [
          clientAccountProfileObjectMetadataForNavigation,
          clientStakeholderRoleObjectMetadataForNavigation,
        ],
    ...
    const allFlatEntityOperationByMetadataName = {
        ...
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatObjectMetadata>({
            ...
            universalIdentifiers:
              CLIENT_CRM_OBJECT_METADATA_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      fieldMetadata: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatFieldMetadata>({
            ...
            universalIdentifiers:
              CLIENT_CRM_FIELD_METADATA_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      index: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatIndexMetadata>({
            ...
            universalIdentifiers: CLIENT_CRM_INDEX_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      view: {
        flatEntityToCreate: getStandardFlatEntitiesToCreateOrThrow<FlatView>({
          ...
          universalIdentifiers: CLIENT_CRM_VIEW_UNIVERSAL_IDENTIFIERS,
        }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      viewFieldGroup: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatViewFieldGroup>({
            ...
            universalIdentifiers:
              CLIENT_CRM_VIEW_FIELD_GROUP_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      viewField: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatViewField>({
            ...
            universalIdentifiers: CLIENT_CRM_VIEW_FIELD_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayout: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayout>({
            ...
            universalIdentifiers: CLIENT_CRM_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayoutTab: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayoutTab>({
            ...
            universalIdentifiers:
              CLIENT_CRM_PAGE_LAYOUT_TAB_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayoutWidget: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayoutWidget>({
            ...
            universalIdentifiers:
              CLIENT_CRM_PAGE_LAYOUT_WIDGET_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
    ...
    if (totalOperationCount === 0) {
      this.logger.log(
        `Client CRM standard metadata already exists for workspace ${workspaceId}, skipping`,
      );
    ...
    if (isDryRun) {
      this.logger.log(
        `[DRY RUN] Would apply ${totalOperationCount} Client CRM standard metadata operations for workspace ${workspaceId}`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      throw new Error(
        `Failed to create Client CRM standard objects for workspace ${workspaceId}: ${JSON.stringify(
    ...
    this.logger.log(
      `Applied ${totalOperationCount} Client CRM standard metadata operations for workspace ${workspaceId}`,
    );
  }
}
