...
const ANALYTICS_OBJECT_NAMES = [
  'analyticsDomainMetric',
  'analyticsMetricSnapshot',
  'analyticsDashboardConfig',
] as const satisfies (keyof typeof STANDARD_OBJECTS)[];

const ANALYTICS_OBJECTS = ANALYTICS_OBJECT_NAMES.map(
  (name) => STANDARD_OBJECTS[name],
);

const ANALYTICS_OBJECT_UNIVERSAL_IDENTIFIERS = ANALYTICS_OBJECTS.map(
  (obj) => obj.universalIdentifier,
);

const ANALYTICS_FIELD_UNIVERSAL_IDENTIFIERS = ANALYTICS_OBJECTS.flatMap(
  (obj) =>
    Object.values(obj.fields).map((f) => f.universalIdentifier),
);

const ANALYTICS_INDEX_UNIVERSAL_IDENTIFIERS = ANALYTICS_OBJECTS.flatMap(
  (obj) =>
    Object.values(obj.indexes).map((i) => i.universalIdentifier),
);

const ANALYTICS_VIEW_UNIVERSAL_IDENTIFIERS = ANALYTICS_OBJECTS.flatMap(
  (obj) =>
    Object.values(obj.views).map((v) => v.universalIdentifier),
);

const ANALYTICS_VIEW_FIELD_GROUP_UNIVERSAL_IDENTIFIERS =
  ANALYTICS_OBJECTS.flatMap((obj) =>
...
const ANALYTICS_VIEW_FIELD_UNIVERSAL_IDENTIFIERS = ANALYTICS_OBJECTS.flatMap(
  (obj) =>
    Object.values(obj.views).flatMap((v) =>
...
const ANALYTICS_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS = [
...
const ANALYTICS_PAGE_LAYOUT_TAB_UNIVERSAL_IDENTIFIERS = Object.values(
  STANDARD_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS.analyticsDomainMetricRecordPage.tabs,
)
...
const ANALYTICS_PAGE_LAYOUT_WIDGET_UNIVERSAL_IDENTIFIERS = Object.values(
  STANDARD_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS.analyticsDomainMetricRecordPage.tabs,
)
  .flatMap((tab) =>
...
@Command({
  ...
  description:
    'Add the analytics semantic layer standard objects (analyticsDomainMetric, analyticsMetricSnapshot, analyticsDashboardConfig) to existing workspaces',
})
export class AddAnalyticsStandardObjectsCommand extends ActiveOrSuspendedWorkspaceCommandRunner {
  constructor(
  ...
  override async runOnWorkspace({
    ...
    this.logger.log(
      `Checking analytics standard objects for workspace ${workspaceId}`,
    );
    ...
    const objectMetadataToCreate =
      getStandardFlatEntitiesToCreateOrThrow<FlatObjectMetadata>({
        ...
        universalIdentifiers: ANALYTICS_OBJECT_UNIVERSAL_IDENTIFIERS,
      });

    if (objectMetadataToCreate.length === 0) {
      this.logger.log(
        `Analytics standard objects already exist for workspace ${workspaceId}, skipping`,
      );
      ...
      fieldMetadata: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatFieldMetadata>({
            ...
            universalIdentifiers: ANALYTICS_FIELD_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      index: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatIndexMetadata>({
            ...
            universalIdentifiers: ANALYTICS_INDEX_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      view: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatView>({
            ...
            universalIdentifiers: ANALYTICS_VIEW_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
        ...
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatViewFieldGroup>({
            ...
            universalIdentifiers:
              ANALYTICS_VIEW_FIELD_GROUP_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      viewField: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatViewField>({
            ...
            universalIdentifiers: ANALYTICS_VIEW_FIELD_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayout: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayout>({
            ...
            universalIdentifiers:
              ANALYTICS_PAGE_LAYOUT_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayoutTab: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayoutTab>({
            ...
            universalIdentifiers:
              ANALYTICS_PAGE_LAYOUT_TAB_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
      pageLayoutWidget: {
        flatEntityToCreate:
          getStandardFlatEntitiesToCreateOrThrow<FlatPageLayoutWidget>({
            ...
            universalIdentifiers:
              ANALYTICS_PAGE_LAYOUT_WIDGET_UNIVERSAL_IDENTIFIERS,
          }),
        flatEntityToDelete: [],
        flatEntityToUpdate: [],
      },
    ...
    if (totalCreateCount === 0) {
      this.logger.log(
        `Analytics standard objects already fully exist for workspace ${workspaceId}, skipping`,
      );
    ...
    if (validateAndBuildResult.status === 'fail') {
      this.logger.error(
        `Failed to create analytics standard objects:\n${JSON.stringify(validateAndBuildResult, null, 2)}`,
      );

      throw new Error(
        `Failed to create analytics standard objects for workspace ${workspaceId}`,
      );
    }

    this.logger.log(
      `Successfully created ${totalCreateCount} analytics standard metadata entities for workspace ${workspaceId}`,
    );
  }
}
