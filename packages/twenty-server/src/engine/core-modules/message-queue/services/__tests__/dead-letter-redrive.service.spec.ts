...
describe('DeadLetterRedriveService', () => {
  ...
  describe('redrive', () => {
    it('should re-enqueue a failed job with the correct idempotencyKey', async () => {
      const failedJob = createFailedJob();
      mockQueue.getJobs.mockResolvedValue([failedJob]);

      await service.redrive({ queueName });

      // Verify re-add was called with idempotencyKey set to the original job id
    ...
    it('should be idempotent: calling redrive twice re-adds the same jobs (BullMQ dedup handles it)', async () => {
      ...
      // The idempotencyKey is the same, so BullMQ dedupes the second add.
      expect(mockQueue.add).toHaveBeenCalledTimes(2);
