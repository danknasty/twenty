export const STANDARD_OBJECT_ICONS = {
  activityTarget: 'IconCheckbox',
  activity: 'IconCheckbox',
  apiKey: 'IconRobot',
  attachment: 'IconFileImport',
  blocklist: 'IconForbid2',
  behavioralEvent: 'IconTimelineEvent',
  calendarChannelEventAssociation: 'IconCalendar',
  calendarEventParticipant: 'IconCalendar',
  calendarEvent: 'IconCalendar',
  callRecording: 'IconVideo',
  comment: 'IconMessageCircle',
  company: 'IconBuildingSkyscraper',
  dashboard: 'IconLayoutDashboard',
  externalEntityLink: 'IconLink',
  inboundEventLedger: 'IconInboxArrowDown',
  outboundEventLedger: 'IconInboxArrowUp',
  auditLog: 'IconTimelineEvent',
  messageChannelMessageAssociation: 'IconMessage',
  messageParticipant: 'IconUserCircle',
  messageThread: 'IconMessage',
  messageThreadSubscriber: 'IconPerson',
  message: 'IconMessage',
  note: 'IconNotes',
  noteTarget: 'IconCheckbox',
  opportunity: 'IconTargetArrow',
  person: 'IconUser',
  task: 'IconCheckbox',
  taskTarget: 'IconCheckbox',
  timelineActivity: 'IconTimelineEvent',
  viewField: 'IconTag',
  viewGroup: 'IconTag',
  viewFilter: 'IconFilterBolt',
  viewSort: 'IconArrowsSort',
  view: 'IconLayoutCollage',
  webhook: 'IconRobot',
  workflow: 'IconSettingsAutomation',
  workflowEventListener: 'IconSettingsAutomation',
  workflowRun: 'IconHistoryToggle',
  workflowVersion: 'IconVersions',
  workflowAutomatedTrigger: 'IconSettingsAutomation',
  workspaceEventOutbox: 'IconMailForward',
  workspaceMember: 'IconUserCircle',
confidentialityRecord: 'IconLock',
  conflictCheck: 'IconShieldCheck',
  executiveProfile: 'IconUserStar',
  executiveCareerExperience: 'IconBriefcase',
  executiveEducation: 'IconSchool',
  executiveBoardService: 'IconBuilding',
  executiveCapability: 'IconStars',
  executiveLanguage: 'IconLanguage',
  executiveArtifact: 'IconFile',
  executiveAward: 'IconAward',
  executiveExternalProfile: 'IconWorld',
  executiveSearchPreference: 'IconSettings',
  marketMap: 'IconMap',
  offLimitsRestriction: 'IconForbid2',
  relationshipEdge: 'IconShare2',
  researchCandidate: 'IconUserSearch',
  researchStrategy: 'IconChartDots',
  targetCompany: 'IconBuildingSkyscraper',,
externalEntityLink: 'IconLink',
  executiveProfile: 'IconUserCircle',
} as const;
