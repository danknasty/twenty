...
describe('BoardMatrixEvaluationService', () => {
  ...
  it('should successfully evaluate a candidate against criteria', async () => {
    ...
    const criteria = [
      { id: 'crit-1', name: 'Financial Expertise', category: 'FINANCIAL_EXPERTISE', weight: 0.4, description: 'CFO', isRequired: true, boardCompositionProfileId: 'profile-1' },
      { id: 'crit-2', name: 'Industry', category: 'INDUSTRY_KNOWLEDGE', weight: 0.6, description: 'SaaS', isRequired: true, boardCompositionProfileId: 'profile-1' },
    ];

    const mockProfileRepo = { findOne: jest.fn().mockResolvedValue({ id: 'profile-1', name: 'Tech Board' }) };
    const mockCandidacyRepo = { findOne: jest.fn().mockResolvedValue({ id: 'candidacy-1' }) };
    const mockCriterionRepo = { find: jest.fn().mockResolvedValue(criteria) };
    const mockEvalRepo = { save: jest.fn().mockResolvedValue({}), find: jest.fn().mockResolvedValue([]) };
    ...
    const result = await service.evaluateCandidate(mockInput, mockWorkspaceId, mockAuthContext);
  ...
  it('should update evaluation records with human review timestamp', async () => {
    ...
    mocks.globalWorkspaceOrmManager.getRepository.mockResolvedValue(mockEvalRepo);

    await service.markHumanReviewComplete('candidacy-1', mockWorkspaceId, mockAuthContext);
